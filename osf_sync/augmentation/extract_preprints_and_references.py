...
class TEIExtractor:
    ...
    def parse_file(self, file_path: str) -> Dict:
        ...
        try:
            ...
            with_doi = sum(
                1 for ref in result['references'] if ref.get('has_doi'))
            ...
            for author in author_elements:
                try:
                    surname = author.xpath(
                        './/tei:surname/text()', namespaces=self.ns)
                    forename = author.xpath(
                        './/tei:forename/text()', namespaces=self.ns)
    ...
    def _extract_reference_data(self, biblstruct) -> Optional[Dict]:
        ...
        try:
            ...
            # Extract DOI
            dois = biblstruct.xpath(
                './/tei:idno[@type="DOI"]/text()', namespaces=self.ns)
            if dois:
                for doi in dois:
                    if doi.strip().startswith("10."):
                        ref['doi'] = doi.strip()
            ...
            # Extract journal
            journals = biblstruct.xpath(
                './/tei:monogr/tei:title[@level="j"]/text()', namespaces=self.ns)
            if journals:
            ...
            raw_notes = biblstruct.xpath(
                './/tei:note[@type="raw_reference"]/text()', namespaces=self.ns)
...
def print_summary(results: List[Dict]):
    ...
    total_refs_with_doi = sum(
        r['stats']['references_with_doi'] for r in results)
    total_refs_without_doi = sum(
        r['stats']['references_without_doi'] for r in results)

    preprints_with_doi = sum(
        1 for r in results if r['preprint'].get('has_doi'))
    preprints_with_title = sum(
        1 for r in results if r['preprint'].get('has_title'))
    ...
    print(
        f"  With DOI: {total_refs_with_doi} ({total_refs_with_doi/total_references*100:.1f}%)")
    print(
        f"  Without DOI: {total_refs_without_doi} ({total_refs_without_doi/total_references*100:.1f}%)")
    print(f"{'='*80}\n")
